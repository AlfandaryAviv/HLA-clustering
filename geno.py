...
def change_races(graph):
        dict_races = {}
        races_dict = {}

        with open('table') as ta:
            for line in ta:
                line = line.strip().split(',')
                dict_races[line[0]] = line[1]

        for node in graph.nodes():
            ids = graph.nodes[node]['ids']
            for id in ids:
                race1 = dict_races[ids[id][0]]
                race2 = dict_races[ids[id][1]]
                graph.nodes[node]['ids'][id] = (race1, race2 )
                races_dict[race1] = races_dict.get(race1, 0) + 1
                races_dict[race2] = races_dict.get(race2, 0) + 1
        return races_dict
...
# total: max/count_all
def most_pop(com_race_count,f_results,races_dict,weights,resolution):
    t=time.time()
    count_maxs=0
    count_all=0
    top_race = dict()
    ...
    races_list = list(races_dict.keys())
    list_true = []
    list_pred = []
    for com, races in com_race_count.items():
        ...
        if com_size[com]>2:
            ...
            for race,apperances_num in dict_apperances.items():
                list_true.extend([races_list.index(race)]*int(apperances_num*2))
                list_pred.extend([races_list.index(most_common[0][0])]*int(apperances_num*2))
    if count_all>0:
    ...
    f1_macro = f1_score(list_true, list_pred, average='macro')
    print(f1_macro)
    f1_micro = f1_score(list_true, list_pred, average='micro')
    print(f1_micro)

    p = []
    sum_vals = sum(races_dict.values())
    for key,val in races_dict.items():
        p.extend([val/sum_vals])
    p = np.random.multinomial(count_all,p , size=1)

    list_pred = []
    for i,num in enumerate(p[0]):
        list_pred.extend([i]*(num*2))

    random.shuffle(list_pred)
    random.shuffle(list_true)
...
def plot_communitues(top_race, com_size, resolution, weights):
    ...
    plt.ylabel('Proportion of each race')
    plt.title('Race proportion within each size group')
    plt.tight_layout()
...
def confusion_matrix(top_race,nodes_data,cd, races,weights,resolution):
    ...
    ax.set_yticks(matplotlib.numpy.arange(len(races_list)))
    ax.set_yticklabels(races_list)
    #ax.set_xticks(matplotlib.numpy.arange(len(races_list)))
    ax.set_xticklabels(['']*len(races_list))
    #ax.tick_params(axis='x', labelsize=7, rotation=90, labelbottom=True, labeltop=False)
...
def check_entropy(com_size, com_labels):
    ...
    plt.title('Entropy per community')
    plt.xlabel("Community size")
    plt.ylabel("Entropy")
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    config = {
        "f_data": params.get("data_file", 'new_data_geno_5_races.txt'),
    ...
    for weights in weights_list:
        for resolution in resolution_list:
            ...
            graph = pickle.load(open("./grid_pkl/graph_1_1", "rb"))
            race_dict = pickle.load(open("./grid_pkl/races_dict_1_1", "rb"))
            # race_dict = change_races(graph)
            print(len(graph.nodes()))
            print(len(race_dict))
            ...
            # cd = pickle.load(open("./grid_pkl_all/cd_"+str(resolution)+"_"+str(weights[0]), "rb"))

            com_nodes = community_nodes(cd,resolution, weights)
            # com_nodes = pickle.load(open("./grid_pkl_all/com_nodes_"+str(resolution)+"_"+str(weights[0]),"rb"))
            print("number of communities:", len(com_nodes))
            com_race_count = com_races(com_nodes, graph.nodes())
            # print(com_race_count)

            top_race, com_size, percent_top_race, com_labels = most_pop(com_race_count,f_results,race_dict,weights,resolution)
